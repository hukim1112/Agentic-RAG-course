...
def web_search(query: str) -> str:
    """
    Performs web search using Tavily Search API if TAVILY_API_KEY is present,
    otherwise gracefully falls back to DuckDuckGo (ddgs).
    """
    ...
    if tavily_key:
        try:
            ...
            if isinstance(raw_res, dict) and "results" in raw_res:
                formatted = []
                for idx, item in enumerate(raw_res["results"], start=1):
            ...
            pass  # Fallback to DuckDuckGo if Tavily call fails
